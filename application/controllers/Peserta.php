    ...
    class Peserta extends CI_CONTROLLER{
        ...
        public function ajax_list($konfirm){
            ...
            foreach ($list as $peserta) {
                ...
                if($konfirm == 1){
                    ...
                    if($peserta->data_login == 1) $row[] = '<center><a target="_blank" href="javascript:void(0)" data-id="'.$peserta->id_user.'|'.$peserta->nama.'|https://api.whatsapp.com/send?phone=62'.substr($peserta->no_hp, 1).'&text=%F0%9F%91%8F%20*Selamat%20'.$nama.'%20Anda%20telah%20terdaftar%20di%20Member%20Area%20mrscholae%20.*%0A%0A*Silahkan%20kunjungi%20'.$link_member['value'].'*%0A%0A*Dengan%20menggunakan%20data%20Login%20berikut%20:%3A*%0AUsername%20%3A%20'.$peserta->username.'%0APassword%20%3A%20'.date('His', strtotime($peserta->tgl_input)).'%0A%0AJika%20ada%20pertanyaan%20lebih%20lanjut%2C%20harap%20menghubungi%20saya%20langsung%20ya%20%F0%9F%98%8A%0A%0A*-%20mrscholae*" id="dataLogin" class="btn btn-sm btn-success"><i class="fa fa-user-lock"></i></a></center>';
                    else $row[] = '<center><a target="_blank" href="javascript:void(0)" data-id="'.$peserta->id_user.'|'.$peserta->nama.'|https://api.whatsapp.com/send?phone=62'.substr($peserta->no_hp, 1).'&text=%F0%9F%91%8F%20*Selamat%20'.$nama.'%20Anda%20telah%20terdaftar%20di%20Member%20Area%20mrscholae%20.*%0A%0A*Silahkan%20kunjungi%20'.$link_member['value'].'*%0A%0A*Dengan%20menggunakan%20data%20Login%20berikut%20:%3A*%0AUsername%20%3A%20'.$peserta->username.'%0APassword%20%3A%20'.date('His', strtotime($peserta->tgl_input)).'%0A%0AJika%20ada%20pertanyaan%20lebih%20lanjut%2C%20harap%20menghubungi%20saya%20langsung%20ya%20%F0%9F%98%8A%0A%0A*-%20mrscholae*" id="dataLogin" class="btn btn-sm btn-secondary"><i class="fa fa-user-lock"></i></a></center>';
                } else {
                    $row[] = '<center>'.$no.'</center>';
                    if($peserta->username == "") $row[] = '<a href="javascript:void(0)" class="btn btn-sm btn-primary" data-id="'.$peserta->id_user.'|'.$peserta->nama.'" id="btnAddId">buat ID</a>';
                    else $row[] = $peserta->username;
        ...
        // get
            public function get_detail_peserta(){
                ...
                $data['link'] = 'https://api.whatsapp.com/send?phone=62'.substr($data["no_hp"], 1).'&text=%F0%9F%91%8F%20*Selamat%20'.$nama.'%20Anda%20telah%20terdaftar%20di%20Member%20Area%20mrscholae%20.*%0A%0A*Silahkan%20kunjungi%20'.$link_member['value'].'*%0A%0A*Dengan%20menggunakan%20data%20Login%20berikut%20:%3A*%0AUsername%20%3A%20'.$data['username'].'%0APassword%20%3A%20'.date('dmY', strtotime($data['tgl_lahir'])).'%0A%0AJika%20ada%20pertanyaan%20lebih%20lanjut%2C%20harap%20menghubungi%20saya%20langsung%20ya%20%F0%9F%98%8A%0A%0A*-%20mrscholae*';
